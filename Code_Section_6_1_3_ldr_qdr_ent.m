...
% - relative average distance to the oracle conditional-optimal entropic policy
% - out-of-sample performance (average conditional entropic risk on a fixed test set)
% - running time
...
% Number of support points used to approximate the true conditional
% distribution when computing the oracle entropic policy
n_oracle_epsilon = 199;
...
fprintf('Comparing RKHS, LDR, and QDR for the contextual entropic-risk newsvendor problem.\n');
...
%% Oracle conditional-optimal entropic policy on a fixed context grid
...
epsilon_oracle = truncatedStandardGrid(n_oracle_epsilon);

fprintf('Constructing the oracle entropic policy on the context grid...\n');
z_opt_ent = trueContextOptEntropic2D(X1_grid, X2_grid, a1, a2, h, b, gamma, epsilon_oracle);
norm_z_opt_ent = norm(z_opt_ent, 'fro');

fprintf('Oracle entropic policy on the context grid has been constructed.\n');
...
fprintf('Constructing the oracle entropic policy on the test contexts...\n');
z_opt_test = trueContextOptEntropic2D(x1_test, x2_test, a1, a2, h, b, gamma, epsilon_oracle);
oracle_oos = averageConditionalEntropicRisk(z_opt_test, y_test, h, b, gamma);

fprintf('Fixed out-of-sample test set has been generated.\n');
fprintf('Oracle out-of-sample performance (average conditional entropic risk): %.6f\n\n', oracle_oos);
...
for i = 1:num_train_sizes
    ...
    % Sample-size-dependent hyperparameters for RKHS
    ...
    for j = 1:num_replications
        ...
        loss_vec = b * diff_yz + h * diff_zy;
        Objective = (1 / gamma) * logsumexp(gamma * loss_vec) - (1 / gamma) * log(n_train) ...
        ...
        if diagnostics.problem == 0
            ...
            distance_rkhs(i, j) = norm(z_grid_rkhs - z_opt_ent, 'fro') / norm_z_opt_ent;
            oos_rkhs(i, j) = averageConditionalEntropicRisk(z_test_rkhs, y_test, h, b, gamma);
        ...
        if diagnostics.problem == 0
            ...
            distance_ldr(i, j) = norm(z_grid_ldr - z_opt_ent, 'fro') / norm_z_opt_ent;
            oos_ldr(i, j) = averageConditionalEntropicRisk(z_test_ldr, y_test, h, b, gamma);
        ...
        if diagnostics.problem == 0
            ...
            distance_qdr(i, j) = norm(z_grid_qdr - z_opt_ent, 'fro') / norm_z_opt_ent;
            oos_qdr(i, j) = averageConditionalEntropicRisk(z_test_qdr, y_test, h, b, gamma);
...
fprintf(['A fixed nested Monte Carlo test set is used for all methods and all replications:\n' ...
         ...
         '- The out-of-sample performance is defined as the average empirical conditional entropic risk over the %d test contexts.\n'], ...
...
fprintf('\n============================================================\n');
fprintf('Mean out-of-sample performance (average conditional entropic risk)\n');
fprintf('============================================================\n');
disp(mean_oos_table);
...
drawnow;
exportgraphics(fig, 'Section_613_rkhs_ldr_qdr_entropic_boxplot.pdf', ...
    'ContentType', 'vector', 'BackgroundColor', 'white');
fprintf('\nSaved grouped boxplot figure to Section_613_rkhs_ldr_qdr_entropic_boxplot.pdf\n');
...
all_surface_values = [z_opt_ent(:); z_grid_ldr_mean(:); z_grid_qdr_mean(:); z_grid_rkhs_mean(:)];
...
nexttile;
surf(X1_grid, X2_grid, z_opt_ent, 'EdgeColor', 'none');
view(245, 20);
...
title('Conditional-entropic-RM policy', 'FontSize', 11);
colormap(parula);
box on;
grid on;
set(gca, 'FontSize', 10);
...
ylabel('$X_2$', 'Interpreter', 'latex');
zlabel('$z$', 'Interpreter', 'latex');
title('Entropic-RM policy with LDR', 'FontSize', 11);
colormap(parula);
box on;
grid on;
set(gca, 'FontSize', 10);
...
ylabel('$X_2$', 'Interpreter', 'latex');
zlabel('$z$', 'Interpreter', 'latex');
title('Entropic-RM policy with QDR', 'FontSize', 11);
colormap(parula);
box on;
grid on;
set(gca, 'FontSize', 10);
...
ylabel('$X_2$', 'Interpreter', 'latex');
zlabel('$z$', 'Interpreter', 'latex');
title('Entropic-RM policy within RKHS', 'FontSize', 11);
colormap(parula);
box on;
grid on;
set(gca, 'FontSize', 10);
...
drawnow;
exportgraphics(fig_surface, 'Section_613_rkhs_ldr_qdr_entropic_surface.pdf', ...
    'ContentType', 'vector', 'BackgroundColor', 'white');
fprintf('Saved 3D policy surface figure to Section_613_rkhs_ldr_qdr_entropic_surface.pdf\n');

%% Local functions
function epsilon_grid = truncatedStandardGrid(num_points)
    u = linspace(0.001, 0.999, num_points);
    cdf_lower = normcdf(-2, 0, 1);
    cdf_upper = normcdf(2, 0, 1);
    u = cdf_lower + u .* (cdf_upper - cdf_lower);
    epsilon_grid = norminv(u, 0, 1);
...
function z_opt = trueContextOptEntropic2D(x1, x2, a1, a2, h, b, gamma, epsilon_support)
    ...
    for idx = 1:numel(mu)
        y_support = max(mu(idx) + sigma(idx) * epsilon_support, 0);
        upper_bound = max(y_support) + 0.1 * (max(y_support) - min(y_support)) + 1;
        objective = @(z) entropicRiskFromSamples(z, y_support, h, b, gamma);
        z_opt(idx) = fminbnd(objective, 0, upper_bound, options);
    end
end

function value = entropicRiskFromSamples(z, y_support, h, b, gamma)
    loss = h * max(z - y_support, 0) + b * max(y_support - z, 0);
    value = (1 / gamma) * log(mean(exp(gamma * loss)));
end

function avg_ent = averageConditionalEntropicRisk(z_policy, y_matrix, h, b, gamma)
    ...
    ent_each_context = (1 / gamma) * log(mean(exp(gamma * loss), 1));
    avg_ent = mean(ent_each_context);
end
